...
namespace CMCS.DumblyConcealer.Tasks.BeltSampler
{
	...
	public class EquBeltSamplerDAO
	{
		...
		public int SyncSignal(Action<string, eOutputType> output)
		{
			...
			foreach (EquPDCYJSignal entity in this.EquDber.Entities<EquPDCYJSignal>())
			{
		...
		public void SyncPlan(Action<string, eOutputType> output)
		{
			...
			res = 0;
			// 第三方 > 集中管控
			foreach (EquPDCYJPlan entity in this.EquDber.Entities<EquPDCYJPlan>("where DataFlag=2 and trunc(createdate)=trunc(sysdate)"))
			{
		...
		public void SyncUnloadCmd(Action<string, eOutputType> output)
		{
			...
			res = 0;
			// 第三方 > 集中管控
			foreach (EquPDCYJUnloadCmd entity in this.EquDber.Entities<EquPDCYJUnloadCmd>("where DataFlag=2 and trunc(CreateDate)=trunc(sysdate)"))
			{
