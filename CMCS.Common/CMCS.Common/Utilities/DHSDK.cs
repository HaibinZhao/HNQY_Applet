...
namespace CMCS.Common.Utilities
{
    public sealed class DHSDK
    {
        ...
        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Ansi)]
        public extern static IntPtr DPSDK_Create(dpsdk_sdk_type_e nType, ref IntPtr nPDLLHandle);

        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Ansi)]
        public extern static IntPtr DPSDK_Destroy(IntPtr nPDLLHandle);

        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Ansi)]
        public extern static IntPtr DPSDK_SetLog(IntPtr nPDLLHandle, dpsdk_log_level_e nLevel, [MarshalAs(UnmanagedType.LPStr)] string szFilename, Boolean bScreen, Boolean bDebugger);

        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Ansi)]
        public extern static IntPtr DPSDK_StartMonitor(IntPtr nPDLLHandle, [MarshalAs(UnmanagedType.LPStr)] string szFilename);
        ...
        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Unicode)]
        ...
        [DllImport(@"DH\DPSDK_Ext.dll", CharSet = CharSet.Ansi)]
        ...
        [DllImport(@"DH\DPSDK_Ext.dll", CharSet = CharSet.Ansi)]
        private extern static IntPtr DPSDK_PicCapture(IntPtr nPDLLHandle, IntPtr nSeq, dpsdk_pic_type_e nPicType, [MarshalAs(UnmanagedType.LPStr)] string szFilename);
        ...
        //按文件下载录像，存储成dav格式
        [DllImport(@"DH\DPSDK_Ext.dll", CharSet = CharSet.Ansi)]
        private extern static IntPtr DPSDK_DownloadRecordByFile(IntPtr nPDLLHandle, out int nPlaybackSeq, byte[] szCameraId, ref Single_Record_Info_t pSingleRecord, byte[] szFileName, IntPtr nTimeout);

        //按文件下载录像，存储成其他格式
        [DllImport(@"DH\DPSDK_Ext.dll", CharSet = CharSet.Ansi)]
        private extern static IntPtr DPSDK_DownloadRecordByFileEx(IntPtr nPDLLHandle, out int nPlaybackSeq, byte[] szCameraId, ref Single_Record_Info_t pSingleRecord, byte[] szFileName, file_type_e nFileType, IntPtr nTimeout);
        ...
        //定位录像
        [DllImport(@"DH\DPSDK_Ext.dll", CharSet = CharSet.Ansi)]
        private extern static IntPtr DPSDK_SeekPlaybackBySeq(IntPtr nPDLLHandle, int nPlaybackSeq, UInt64 seekBegin, UInt64 seekEnd, IntPtr nTimeout);

        //定位本地录像
        [DllImport(@"DH\DPSDK_Ext.dll", CharSet = CharSet.Ansi)]
        private extern static IntPtr DPSDK_SeekLocalPlaybackBySeq(IntPtr nPDLLHandle, int nPlaybackSeq, int nPos, IntPtr nTimeout);
        ...
        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Ansi, EntryPoint = "DPSDK_SetGeneralJsonTransportCallbackEx", ExactSpelling = false, CallingConvention = CallingConvention.StdCall)]
        ...
        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Ansi)]
        private extern static IntPtr DPSDK_GetTvWallList(IntPtr nPDLLHandle, ref TvWall_List_Info_t pTvWallListInfo);
        ...
        [DllImport(@"DH\DPSDK_Core.dll", CharSet = CharSet.Ansi)]
        private extern static IntPtr DPSDK_PtzLockCamera(IntPtr nPDLLHandle, ref Ptz_Prepoint_Operation_Info_t pPrepoint, IntPtr nTimeout);
        ...
        public static bool Logion(string szIp, Int32 nPort, string szUsername, string szPassword, IntPtr nPDLLHandle)
        {
            //DPSDK_Login
            Login_Info_t loginInfo = new Login_Info_t();
            loginInfo.szIp = szIp;
            loginInfo.nPort = (uint)nPort;
            loginInfo.szUsername = szUsername;
            loginInfo.szPassword = szPassword;
        ...
        public static bool StartPreview(IntPtr previewHandle, string szCameraId, IntPtr nPDLLHandle, IntPtr realseq)
        {
            IntPtr handle = previewHandle;
            Get_RealStream_Info_t Info = new Get_RealStream_Info_t();
            Info.szCameraId = szCameraId;
            Info.nRight = dpsdk_check_right_e.DPSDK_CORE_NOT_CHECK_RIGHT;
            Info.nStreamType = dpsdk_stream_type_e.DPSDK_CORE_STREAMTYPE_MAIN;
            Info.nMediaType = dpsdk_media_type_e.DPSDK_CORE_MEDIATYPE_VIDEO;
            Info.nTransType = dpsdk_trans_type_e.DPSDK_CORE_TRANSTYPE_TCP;
            IntPtr result = DPSDK_StartRealplay(nPDLLHandle, out realseq, ref Info, handle, (IntPtr)10000);
        ...
        public static bool closevideo(IntPtr nPDLLHandle, IntPtr realseq)
        {
            //DPSDK_PicCapture(nPDLLHandle, realseq, dpsdk_pic_type_e.DPSDK_CORE_PIC_FORMAT_JPEG, "D:\\test.jpg");
            IntPtr result = DPSDK_StopRealplayBySeq(nPDLLHandle, realseq, (IntPtr)10000);
