...
namespace CMCS.CommonADGS.Core
{
	...
	public class OracleSqlBuilder
	{
		...
		/// <param name="dtl"></param>
		/// <returns></returns>
		public static string BuildTableSQL(string tableName, DataColumnCollection dtlColumns)
		{
			StringBuilder sql = new StringBuilder();
			sql.AppendFormat("CREATE TABLE {0} (", tableName);
			...
			foreach (DataColumn column in dtlColumns)
			{
				switch (column.DataType.ToString())
				{
					case "System.String":
		...
		/// <param name="type"></param>
		/// <returns></returns>
		public static string ToDbValue(string value, string type)
		{
			switch (type)
			{
				case "System.String":
		...
		/// <returns></returns>
		public static List<DataColumnList> ColumnToColumnList(DataColumnCollection columns)
		{
			List<DataColumnList> list = new List<DataColumnList>();
			foreach (DataColumn column in columns)
			{
				DataColumnList entity = new DataColumnList();
				entity.ColumnName = column.ColumnName;
				entity.DataType = column.DataType.ToString();
		...
		/// <param name="dtl"></param>
		/// <returns></returns>
		public static string BuildTableSQL(string tableName, List<DataColumnList> dtl)
		{
			StringBuilder sql = new StringBuilder();
			sql.AppendFormat("CREATE TABLE {0} (", tableName);
			...
			foreach (DataColumnList column in dtl)
			{
		...
		/// <param name="columns"></param>
		/// <returns></returns>
		public static List<DataRowList> ColumnToColumnList(DataRow dr)
		{
			List<DataRowList> list = new List<DataRowList>();
			foreach (DataColumn column in dr.Table.Columns)
			{
				DataRowList entity = new DataRowList();
				entity.ColumnName = column.ColumnName;
				entity.DataType = column.DataType.ToString();
				entity.ColumnValue = ToDbValue(dr[column.ColumnName].ToString(), column.DataType);
		...
		/// <param name="dr"></param>
		/// <returns></returns>
		public static string BuildPrimaryKeyValue(string primaryKeys, List<DataRowList> dr)
		{
			string res = string.Empty;

			if (string.IsNullOrEmpty(primaryKeys)) throw new ArgumentException(primaryKeys);

			string[] keys = primaryKeys.Trim().Trim('|').ToUpper().Split('|');
			if (keys.Length == 0) throw new ArgumentException(primaryKeys);

			foreach (DataRowList item in dr)
			{
				if (keys.Contains(item.ColumnName.ToUpper()))
					res += item.ColumnValue + "-";
			}
			return res.TrimEnd('-');
		}
		...
		/// <param name="dr"></param>
		/// <returns></returns>
		public static string BuildInsertSQL(string tableName, string primaryKeyValue, string machineCode, List<DataRowList> dr)
		{
			StringBuilder strbColumn = new StringBuilder();
			StringBuilder strbValue = new StringBuilder();

			foreach (DataRowList column in dr)
			{
				switch (column.DataType)
				{
					case "System.String":
						strbColumn.AppendFormat("{0},", RelieveOracleKeywords(column.ColumnName.ToUpper()));
						strbValue.AppendFormat("'{0}',", ToDbValue(column.ColumnValue, column.DataType));
						break;
					case "System.DateTime":
						strbColumn.AppendFormat("{0},", RelieveOracleKeywords(column.ColumnName.ToUpper()));
						strbValue.AppendFormat("to_date('{0}','yyyy/mm/dd HH24:MI:SS'),", ToDbValue(column.ColumnValue, column.DataType));
						break;
					case "System.Int16":
					case "System.Int32":
					case "System.Int64":
		...
		/// <param name="dr"></param>
		/// <returns></returns>
		public static string BuildUpdateSQL(string tableName, string primaryKeyValue, string facilityNumber, List<DataRowList> dr)
		{
			StringBuilder strbUpdate = new StringBuilder();
			...
			foreach (DataRowList column in dr)
			{
				...
				{
					case "System.String":
						strbUpdate.AppendFormat("{0}='{1}',", RelieveOracleKeywords(column.ColumnName.ToUpper()), ToDbValue(column.ColumnValue, column.DataType));
						break;
					case "System.DateTime":
						strbUpdate.AppendFormat("{0}=to_date('{1}','yyyy/mm/dd HH24:MI:SS'),", RelieveOracleKeywords(column.ColumnName.ToUpper()), ToDbValue(column.ColumnValue, column.DataType));
						break;
					case "System.Int16":
					case "System.Int32":
					case "System.Int64":
