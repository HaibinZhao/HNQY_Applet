...
namespace CMCS.CarTransport.BeltSampler.Core
{
    public class PreviewCarBmp
    {
        ...
        public Bitmap GetPreviewBitmap(Bitmap bCar, int imageWidth, int imageHeight)
        {
            ...
            // 车厢长
            float carriageLength = this.CurrTruck.CarriageLength * zoomRate;
            // 车总长
            float truckTotalLength = imageWidth;

            // 车厢尾部到第1根拉筋距离
            float leftFromTailObstacle1 = this.CurrTruck.LeftFromTailObstacle1 * zoomRate;
            // 车厢尾部到第2根拉筋距离
            float leftFromTailObstacle2 = this.CurrTruck.LeftFromTailObstacle2 * zoomRate;
            // 车厢尾部到第3根拉筋距离
            float leftFromTailObstacle3 = this.CurrTruck.LeftFromTailObstacle3 * zoomRate;
            // 车厢尾部到第4根拉筋距离
            float leftFromTailObstacle4 = this.CurrTruck.LeftFromTailObstacle4 * zoomRate;
            // 车厢尾部到第5根拉筋距离
            float leftFromTailObstacle5 = this.CurrTruck.LeftFromTailObstacle5 * zoomRate;
            // 车厢尾部到第6根拉筋距离
            float leftFromTailObstacle6 = this.CurrTruck.LeftFromTailObstacle6 * zoomRate;

            // 车厢尾部到第1根拉筋距离
            float rightFromTailObstacle1 = this.CurrTruck.RightFromTailObstacle1 * zoomRate;
            // 车厢尾部到第2根拉筋距离
            float rightFromTailObstacle2 = this.CurrTruck.RightFromTailObstacle2 * zoomRate;
            // 车厢尾部到第3根拉筋距离
            float rightFromTailObstacle3 = this.CurrTruck.RightFromTailObstacle3 * zoomRate;
            // 车厢尾部到第4根拉筋距离
            float rightFromTailObstacle4 = this.CurrTruck.RightFromTailObstacle4 * zoomRate;
            // 车厢尾部到第5根拉筋距离
            float rightFromTailObstacle5 = this.CurrTruck.RightFromTailObstacle5 * zoomRate;
            // 车厢尾部到第6根拉筋距离
            float rightFromTailObstacle6 = this.CurrTruck.RightFromTailObstacle6 * zoomRate;
            ...
            // 绘制坐标点
            Font pointFont = new Font("微软雅黑", (float)Math.Floor(Math.Max(6, this.CurrTruck.AiguilleRadius * zoomRate)), FontStyle.Regular);
            for (int i = 0; i < CurrPoints.Count; i++) g.DrawString((i + 1).ToString(), pointFont, Brushes.Red, padding + xOffest + truckTotalLength - CurrPoints[i].X * zoomRate, CurrPoints[i].Y * zoomRate + yOffest);
